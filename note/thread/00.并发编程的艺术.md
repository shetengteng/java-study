...
protected final boolean tryAcquire(int acquires) { // 公平锁，这里参数是1
    ...
    if (c == 0) {
        if (isFirst(current) && compareAndSetState(0, acquires)) { // 使用cas设置为1，其他线程获取失败
...
- 公平锁和非公平锁释放时，最后都要写一个volatile的变量state

- 公平锁获取时，首先去读取volatile的state

- 非公平锁获取时，使用CAS更新volatile变量
